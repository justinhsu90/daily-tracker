...
public class FB_Facebook {
	 ...
	 */
	public FB_Facebook(String appId) {
		if (appId == null) {
		...
		dialog(activity, LOGIN, params, new DialogListener() {
			...
			public void onComplete(Bundle values) {
				...
				if (isSessionValid()) {
					...
					mAuthDialogListener.onFacebookError(new FB_FacebookError("Failed to receive access token."));
				}
			}
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				if (error != null) {
					if (error.equals(SINGLE_SIGN_ON_DISABLED) || error.equals("AndroidAuthKillSwitchException")) {
						...
						Log.d("Facebook-authorize", "Login failed: " + error);
						mAuthDialogListener.onFacebookError(new FB_FacebookError(error));
					}
				...
				if (data != null) {
					...
					mAuthDialogListener.onError(new FB_DialogError(data.getStringExtra("error"), data.getIntExtra("error_code", -1), data
			...
			public void handleMessage(Message msg) {
				...
				if (token != null) {
					...
					if (msg.getData().containsKey("error_code")) {
						...
						serviceListener.onFacebookError(new FB_FacebookError(error, null, errorCode));
					} else {
	...
	public String request(String graphPath, Bundle params, String httpMethod) throws FileNotFoundException, MalformedURLException, IOException {
		...
		return FB_Util.openUrl(url, httpMethod, params);
	}
	...
	public void dialog(Context context, String action, Bundle parameters, final DialogListener listener) {
		...
		String url = endpoint + "?" + FB_Util.encodeUrl(parameters);
		if (context.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			FB_Util.showAlert(context, "Error", "Application requires permission to access the Internet");
		} else {
			new FB_FbDialog(context, url, listener).show();
		}
	}
	...
	public static interface DialogListener {
		 ...
		 * 
		 */
		public void onFacebookError(FB_FacebookError e);
		 ...
		 * 
		 */
		public void onError(FB_DialogError e);
