...
public final class FB_Util {
	 ...
	 */
	public static JSONObject parseJson(String response) throws JSONException, FB_FacebookError {
		// Edge case: when sending a POST request to /[post_id]/likes
		// the return value is 'true' or 'false'. Unfortunately
		// these values cause the JSONObject constructor to throw
		// an exception.
		if (response.equals("false")) {
			throw new FB_FacebookError("request failed");
		}
		...
		if (json.has("error")) {
			...
			throw new FB_FacebookError(error.getString("message"), error.getString("type"), 0);
		}
		if (json.has("error_code") && json.has("error_msg")) {
			throw new FB_FacebookError(json.getString("error_msg"), "", Integer.parseInt(json.getString("error_code")));
		}
		if (json.has("error_code")) {
			throw new FB_FacebookError("request failed", "", Integer.parseInt(json.getString("error_code")));
		}
		if (json.has("error_msg")) {
			throw new FB_FacebookError(json.getString("error_msg"));
		}
		if (json.has("error_reason")) {
			throw new FB_FacebookError(json.getString("error_reason"));
		}
		return json;
	}
